...
const substituaX = (name) => {
  ...
  for (let w = 0; w < novaFrase.length; w += 1) {
    if (novaFrase[w] === 'x') {
      novaFrase[w] = name;
    }
  }
...
const minhasSkills = (substituaX) => {
  ...
  let resultSkills = 'Minhas três principais habilidades são: ';
  for (let i = 0; i < skills.length; i += 1) {
    if(i === (skills.length - 1)) {
      resultSkills += `${skills[i]}.`
    } else {
      resultSkills += `${skills[i]}, `
    }
  }
...
console.log('');
console.log(minhasSkills(substituaX('José')));