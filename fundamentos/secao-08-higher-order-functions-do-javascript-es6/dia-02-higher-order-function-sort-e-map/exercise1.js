...
const allBooks = books.map(e => `${e.name} - ${e.genre} - ${e.author.name}`);
console.log(allBooks);

// Crie um array com strings no formato NOME_DA_PESSOA_AUTORA - DATA DE NASCIMENTO
const authorsInfo = books.map(e => `${e.author.name} - ${e.author.birthYear}`);
console.log('');
...
const authorsAgeWhenPublished = books.map((e) => (
  {author: e.author.name, age: (e.releaseYear - e.author.birthYear)}
  ))
  .sort((a, b) => a.age - b.age);
console.log('');
console.log(authorsAgeWhenPublished);